...
The superuser live-monitoring surface is user-centric: show one entry per connected participant, provide an actual on-demand preview of the selected participant, and provide a separate two-way Connect action that creates a dedicated superuser-user room. Keep both operations superuser-only, short-lived, justified, and audited.
